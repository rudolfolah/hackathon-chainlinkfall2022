...
export function Settings() {
  ...
  const { data: truflationIndexValue, isLoading: truflationIndexValueLoading } =
    useContractRead({
  ...
  const { data: riskIndexValue, isLoading: riskIndexValueLoading } =
    useContractRead({
      address: contract.contractAddress,
      abi: contract.contractAbi,
  ...
  const { config: configContractWriteRequestUpdateLoanConfig } =
    usePrepareContractWrite({
      address: contract.contractAddress,
      abi: contract.contractAbi,
  ...
  const { write: contractWriteRequestUpdateLoanConfig } = useContractWrite(
    configContractWriteRequestUpdateLoanConfig
  );
      ...
      <Box textAlign={"center"} overflowWrap={"anywhere"} fontSize={13}>
        ...
        <Text>
          {nftBalanceLoading ? (
            ...
            `Owns ${nftBalance?.toString()} HONEYBEE LOANS Prototype NFTs`
          )}
        </Text>
        <Text>
          {tokenBalanceLoading ? (
            ...
            `${tokenBalance?.toString()} HPT`
          )}
        </Text>
      </Box>
      <Box fontSize={13}>
        {!truflationIndexValueLoading &&
          truflationIndexValue &&
          truflationIndexValue.length > 0 && (
            <Text>{truflationIndexValue}</Text>
          )}
        ...
        <Button onClick={() => contractWriteRequestUpdateLoanConfig?.()}>
          Update
        </Button>
      </Box>
      <Box>
        <Text>
          Loan Amount and Interest Rate Range based on NFT Index Value
        </Text>
        <BarChart
          width={325}
          height={225}
